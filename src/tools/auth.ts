    ...
    try {
      return await Promise.race([
        (async () => {
          ...
          // Promise.race doesn't cancel the losing branch. If /settings only
          // resolved after the timeout already fired, stop here instead of
          // making a further, unobserved getCurrentUser() request.
          if (timedOut) {
          ...
          // userLine is assigned on both branches; rolesLine keeps its default
          // when getCurrentUser is blocked (the catch doesn't touch it).
          let userLine: string;
          let rolesLine = "N/A";
          try {
