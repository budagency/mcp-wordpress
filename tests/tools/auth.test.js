...
describe("AuthTools", () => {
  ...
  describe("handleTestAuth", () => {
    ...
    it("preserves statusCode/code from a typed client error instead of discarding them", async () => {
      // Upstream #209 regression test, re-pointed from ping() to the /settings probe:
      // Bud's handleTestAuth uses getSiteSettings as the primary auth check.
    ...
    it("should time out after 10 seconds and not hang indefinitely", async () => {
      vi.useFakeTimers();
      try {
        // Simulate an unreachable site: the primary probe (/settings) never resolves
        mockClient.getSiteSettings.mockImplementation(() => new Promise(() => {}));
    ...
    it("should not call getCurrentUser if the probe resolves after the timeout already fired", async () => {
      vi.useFakeTimers();
      try {
        let resolveSettings;
        mockClient.getSiteSettings.mockImplementation(
          () =>
            new Promise((resolve) => {
        ...
        // /settings finally resolves, but only after the race already settled;
        // the timedOut guard must stop us before getCurrentUser.
        resolveSettings({});
